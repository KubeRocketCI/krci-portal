...
describe("authLoginProcedure", () => {
  ...
  it("should throw error if input URL is invalid", async () => {
    ...
    expect(openIdClient.buildAuthorizationUrl).not.toHaveBeenCalled();
  });
  ...
  it("should handle OIDC discovery failure", async () => {
    ...
    vi.spyOn(openIdClient, "discovery").mockRejectedValueOnce(
    ...
    expect(openIdClient.randomState).not.toHaveBeenCalled();
    expect(openIdClient.buildAuthorizationUrl).not.toHaveBeenCalled();
  });
});
