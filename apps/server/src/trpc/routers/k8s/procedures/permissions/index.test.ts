...
describe("k8sGetResourcePermissions", () => {
  ...
  it("should call createSelfSubjectAccessReview for each verb and return correct result", async () => {
    ...
    expect(mockAuthApi.createSelfSubjectAccessReview).toHaveBeenCalledTimes(
      defaultPermissionsToCheck.length
    );
  ...
  it("should throw validation error for missing input fields", async () => {
    ...
    const caller = createCaller(mockContext);
    await expect(
      caller.k8s.itemPermissions(
        invalidInput as inferProcedureInput<typeof k8sGetResourcePermissions>
      )
  ...
  it("should throw error if Kubernetes API call fails", async () => {
    ...
    mockAuthApi.createSelfSubjectAccessReview.mockRejectedValueOnce(
      new Error("Kubernetes API unavailable")
    );

    const caller = createCaller(mockContext);

    await expect(caller.k8s.itemPermissions(input)).rejects.toThrow(
