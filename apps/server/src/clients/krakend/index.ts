...
export class KrakendClient {
  ...
  private async fetchJson<DataType>(
    path: string,
    init?: RequestInit
  ): Promise<DataType> {
  ...
  async getDepTrackProjectMetrics(
    projectID: string
  ): Promise<DepTrackProjectMetrics> {
  ...
  async getSonarQubeMetrics(componentName: string): Promise<SonarQubeMetrics> {
    ...
    return this.fetchJson<SonarQubeMetrics>(
  ...
  async getGitFusionRepositories(
    gitServer: string,
    owner: string
  ): Promise<GitFusionRepositoryListResponse> {
    const params = new URLSearchParams({
      gitServer,
      owner,
    });
  ...
  async getGitFusionOrganizations(
    gitServer: string
  ): Promise<GitFusionOrganizationListResponse> {
    const params = new URLSearchParams({
      gitServer,
    });
  ...
  async getGitFusionBranches(
    gitServer: string,
    owner: string,
    repoName: string
  ): Promise<GitFusionBranchListResponse> {
    const params = new URLSearchParams({
      gitServer,
      owner,
      repoName,
