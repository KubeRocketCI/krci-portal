...
export const MultiFormContextProvider = <FormName extends string>({
    ...
    () =>
      Object.values<MultiFormItem>(forms).reduce<{
        ...
        (acc, formItem) => {
          ...
          if (!acc.isAnyFormDirty && form.formState.dirtyFields && Object.keys(form.formState.dirtyFields).length > 0) {
  ...
  const resetAll = React.useCallback(
    async (resetSharedForm?: () => void) => {
      const resetPromises = Object.values<MultiFormItem>(forms).map((formItem) =>
  ...
  const value = React.useMemo(
    ...
    [forms, resetAll, submitAll, isAnyFormDirty, isAnyFormSubmitting, isAnyFormForbiddenToSubmit, sharedForm]
  );
