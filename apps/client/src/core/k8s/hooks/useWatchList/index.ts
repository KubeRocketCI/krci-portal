...
export const useWatchList = <I extends KubeObjectBase>({
  ...
  const k8sWatchListQueryCacheKey = React.useMemo(
    () => getK8sWatchListQueryCacheKey(clusterName, namespace, resourcePlural),
    [clusterName, namespace, resourcePlural]
  );
  ...
  React.useEffect(() => {
    ...
    return () => {
      console.log("Unsubscribing WebSocket due to useEffect cleanup");
      subscriptionRef.current?.unsubscribe();
      subscriptionRef.current = null;
    };
  }, [query.isFetched, clusterName, namespace, resourcePlural, queryClient, group, version, k8sWatchListQueryCacheKey]);

  return {
    ...query,
    data: query.data || (k8sListInitialData as CustomKubeObjectList<I>),
