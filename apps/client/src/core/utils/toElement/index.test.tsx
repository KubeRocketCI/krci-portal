...
describe("toElement", () => {
  test("returns valid React element as-is with extra props merged", () => {
    ...
    const result = toElement(element, { id: "test-id" }) as React.ReactElement<any>;
  ...
  test("creates element from function component", () => {
    const Component = ({ name, id }: { name?: string; id?: string }) => (
      <div id={id}>{name}</div>
    );
    const result = toElement(Component, { name: "Test", id: "component-id" }) as React.ReactElement<any>;
  ...
  test("creates element from forwardRef component", () => {
    const Component = React.forwardRef<HTMLDivElement, { name?: string }>(
      (_props, ref) => <div ref={ref}>{_props.name}</div>
    );
    const result = toElement(Component, { name: "ForwardRef" }) as React.ReactElement<any>;
  ...
  test("handles empty extra props", () => {
    ...
    const result = toElement(Component) as React.ReactElement<any>;
  ...
  test("merges props correctly for existing element", () => {
    const element = <div className="original" data-test="original">Content</div>;
