...
vi.mock("@my-project/shared", async () => {
  ...
  return {
    ...actual,
    checkIsDefaultBranch: (codebase: Codebase, branch: CodebaseBranch) => {
      return branch.metadata.name === codebase.spec.defaultBranch;
    },
  };
});
...
describe("sortCodebaseBranches", () => {
  test("sorts branches with default branch first", () => {
    ...
    const branches: CodebaseBranch[] = [
      { metadata: { name: "feature-branch" } } as CodebaseBranch,
      { metadata: { name: "main" } } as CodebaseBranch,
      { metadata: { name: "develop" } } as CodebaseBranch,
    ];
  ...
  test("handles case when default branch is not in list", () => {
    ...
    const branches: CodebaseBranch[] = [
      { metadata: { name: "feature-1" } } as CodebaseBranch,
      { metadata: { name: "feature-2" } } as CodebaseBranch,
    ];
...
describe("sortCodebaseBranchesMap", () => {
  test("sorts Map with default branch first", () => {
    ...
    const branchesMap = new Map<string, CodebaseBranch>([
      ["feature-branch", { metadata: { name: "feature-branch" } } as CodebaseBranch],
      ["main", { metadata: { name: "main" } } as CodebaseBranch],
      ["develop", { metadata: { name: "develop" } } as CodebaseBranch],
    ]);
  ...
  test("preserves Map structure with correct keys", () => {
    ...
    const branchesMap = new Map<string, CodebaseBranch>([
      ["branch-1", { metadata: { name: "branch-1" } } as CodebaseBranch],
      ["main", { metadata: { name: "main" } } as CodebaseBranch],
    ]);
