    ...
    () =>
      metrics
        ? transformToChartData(metrics).map((m) => ({
            ...m,
            total: m.components,
            nonVulnerable: m.components - m.vulnerableComponents,
            vulnerable: m.vulnerableComponents,
          }))
        : [],
    [metrics]
  );

  const total = latestMetrics?.components ?? 0;
  const vulnerable = latestMetrics?.vulnerableComponents ?? 0;
