...
export const ApplicationRow = ({ application, index, removeRow }: ApplicationRowProps) => {
  ...
  const applicationsFieldArrayValue = watch(NAMES.ui_applicationsFieldArray);
  const inputDockerStreamsValue = watch(NAMES.inputDockerStreams);
  ...
  const handleDeleteApplicationRow = React.useCallback(() => {
    ...
    setValue(NAMES.ui_applicationsToAddChooser, newAppsList, {
      shouldDirty: false,
    });

    setValue(NAMES.applications, newAppsList, { shouldDirty: false });

    setValue(NAMES.applicationsToPromote, newAppsList, { shouldDirty: false });

    setValue(NAMES.inputDockerStreams, newAppsBranchesList, {
      shouldDirty: false,
    });
