...
export const QualityGatesFieldInline: React.FC = () => {
  const { stage } = useEditStageData();
  const namespace = stage.metadata.namespace!;
  const form = useEditStageForm();

  const qualityGatesFieldValue = useStore(
    form.store,
    (state: typeof form.store.state) => state.values[EDIT_STAGE_FORM_NAMES.qualityGates] || []
  );
  ...
  const qualityGatesFieldMeta = useStore(form.store, (state) => {
    return state.fieldMeta[EDIT_STAGE_FORM_NAMES.qualityGates];
  });
  ...
  const handleSave = (values: QualityGateFormValues) => {
    if (editingGateIndex !== null) {
      ...
      form.setFieldValue(EDIT_STAGE_FORM_NAMES.qualityGates, updated);
    } else {
      // Add new gate
      const newGate: QualityGate = { id: uuidv4(), ...values };
      form.setFieldValue(EDIT_STAGE_FORM_NAMES.qualityGates, [...qualityGatesFieldValue, newGate]);
    }
    setShowGateForm(false);
    setEditingGateIndex(null);
  };
  ...
  const handleDeleteGate = (index: number) => {
    form.setFieldValue(
      EDIT_STAGE_FORM_NAMES.qualityGates,
        ...
        <div className="space-y-4">
          {qualityGatesFieldValue.map((gate, index) => (
            <QualityGateCard
              ...
              gate={gate}
              index={index}
              onEdit={startEditingGate}
              onDelete={handleDeleteGate}
              disabled={showGateForm}
              canDelete={qualityGatesFieldValue.length > 1}
            />
          ))}
        </div>
      )}
