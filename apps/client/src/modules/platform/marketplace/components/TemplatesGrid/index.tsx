...
export const TemplatesGrid = () => {
  ...
  const isLoading = !templatesWatch.query.isFetched || !gitServersWatch.query.isFetched;

  const renderEmptyListComponent = React.useMemo(() => {
    if (isLoading) {
      return null;
    }

    return !hasAtLeastOneGitServer ? (
      <TemplatesWarning />
    ) : templates.length === 0 ? (
      <EmptyList missingItemName={"templates"} icon={<Shop width={128} height={128} fill="#A2A7B7" />} />
    ) : null;
  }, [isLoading, hasAtLeastOneGitServer, templates.length]);
