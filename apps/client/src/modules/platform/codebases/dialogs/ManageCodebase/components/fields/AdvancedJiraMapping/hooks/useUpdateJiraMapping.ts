...
export const useUpdateJiraMapping = ({ setAdvancedMapping, setAdvancedMappingRows }: UseUpdateJiraMapping) => {
  const { watch, setValue } = useFormContext();

  const jiraIssueMetadataPayloadFieldValue = watch(CODEBASE_FORM_NAMES.jiraIssueMetadataPayload.name);
