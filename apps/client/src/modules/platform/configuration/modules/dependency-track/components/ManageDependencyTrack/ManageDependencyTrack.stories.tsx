...
const manageDependencyTrackDecorator = withAppProviders({
...
const meta = {
  title: "Platform/Configuration/DependencyTrack/ManageDependencyTrack",
  component: ManageDependencyTrack,
  parameters: {
    layout: "padded",
  },
  tags: ["autodocs"],
  decorators: [
    manageDependencyTrackDecorator,
    (Story) => (
...
} satisfies Meta<typeof ManageDependencyTrack>;
...
const mockSecret: Secret = {
  ...
  type: "Opaque",
  data: {
    token: btoa("dependency_track_api_token_12345abcdef"),
    url: btoa("https://dependency-track-api.example.com"),
  },
};

/**
 * Create mode - New Dependency Track integration
 * Shows the form with no existing configuration, ready to create a new Dependency Track integration.
 * Dependency Track is an intelligent component analysis platform for identifying and reducing risk in the software supply chain.
 */
 ...
 * Shows the form with an existing Dependency Track integration.
...
export const EditMode: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(canvas.getByLabelText("URL", { selector: "input" })).toHaveValue(
      "https://dependency-track-api.example.com"
    );
    await expect(canvas.getByLabelText("Token", { selector: "input" })).toHaveValue(
      "dependency_track_api_token_12345abcdef"
    );
        ...
        ownerReferences: [
          {
            apiVersion: "v1",
            kind: "EDPComponent",
            name: "dependency-track-component",
            uid: "12345-67890",
...
export const EditModeUndoChanges: Story = {
  ...
  play: async ({ canvas }) => {
    const urlInput = canvas.getByLabelText("URL", { selector: "input" });
    const originalValue = "https://dependency-track-api.example.com";
...
export const EditModeValidation: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await userEvent.tab();
    await expect(canvas.getByText("Enter the external Dependency Track URL.")).toBeInTheDocument();
