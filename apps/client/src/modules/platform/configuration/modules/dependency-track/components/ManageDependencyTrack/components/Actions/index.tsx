...
export const Actions = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result = await trpc.k8s.testIntegrationConnection.mutate({ serviceType: "dependencyTrack", url, token });
      if (cancelled) return;
      ...
      if (result.success) {
        ...
        switch (result.error) {
          ...
          case "NETWORK":
            ...
            break;
          case "HTTP_ERROR":
            setTestError(`Dependency Track responded with status ${result.statusCode}.`);
            break;
        }
      }
    } catch (error) {
        ...
        <div className="my-2">
          {testStatus === "success" && (
            <AlertRoot className="border-green-500 bg-green-50 text-green-800 dark:bg-green-950 dark:text-green-200">
              <CheckCircle2 />
              <AlertDescription>Connection to Dependency Track is working.</AlertDescription>
            </AlertRoot>
          )}
