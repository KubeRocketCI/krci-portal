...
const manageSonarDecorator = withAppProviders({
...
const meta = {
  title: "Platform/Configuration/Sonar/ManageSonar",
  component: ManageSonar,
  parameters: {
    layout: "padded",
  },
  tags: ["autodocs"],
  decorators: [
    manageSonarDecorator,
    (Story) => (
...
} satisfies Meta<typeof ManageSonar>;
...
const mockSecret: Secret = {
  ...
  type: "Opaque",
  data: {
    token: btoa("squ_sonarqube_token_12345abcdef"),
    url: btoa("https://sonarqube-api.example.com"),
  },
};

/**
 * Create mode - New SonarQube integration
 * Shows the form with no existing configuration, ready to create a new SonarQube integration.
 */
 ...
 * Shows the form with an existing SonarQube integration.
...
export const EditMode: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(canvas.getByLabelText("URL", { selector: "input" })).toHaveValue("https://sonarqube-api.example.com");
    await expect(canvas.getByLabelText("Token", { selector: "input" })).toHaveValue("squ_sonarqube_token_12345abcdef");
...
export const EditModeUndoChanges: Story = {
  ...
  play: async ({ canvas }) => {
    const urlInput = canvas.getByLabelText("URL", { selector: "input" });
    const originalValue = "https://sonarqube-api.example.com";
...
export const EditModeValidation: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await userEvent.tab();
    await expect(canvas.getByText("Enter the external SonarQube URL.")).toBeInTheDocument();
