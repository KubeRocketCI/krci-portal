...
export const Actions = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result = await trpc.k8s.testIntegrationConnection.mutate({ serviceType: "sonar", url, token });
      if (cancelled) return;
      ...
      if (result.success) {
        ...
        switch (result.error) {
          ...
          case "NETWORK":
            ...
            break;
          case "HTTP_ERROR":
            setTestError(`SonarQube responded with status ${result.statusCode}.`);
            break;
        }
      }
    } catch (error) {
