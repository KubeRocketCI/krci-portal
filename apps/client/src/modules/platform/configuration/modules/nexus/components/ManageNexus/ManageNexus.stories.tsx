...
const manageNexusDecorator = withAppProviders({
...
const meta = {
  title: "Platform/Configuration/Nexus/ManageNexus",
  component: ManageNexus,
  parameters: {
    layout: "padded",
  },
  tags: ["autodocs"],
  decorators: [
    manageNexusDecorator,
    (Story) => (
...
} satisfies Meta<typeof ManageNexus>;
...
const mockSecret: Secret = {
  ...
  type: "Opaque",
  data: {
    username: btoa("nexus-admin"),
    password: btoa("nexus-secret-password-123"),
    url: btoa("https://nexus-api.example.com"),
  },
};

/**
 * Create mode - New Nexus integration
 * Shows the form with no existing configuration, ready to create a new Nexus integration.
 */
 ...
 * Shows the form with an existing Nexus integration.
 * Displays both QuickLink (external URL) and Secret (username, password, and API URL) configuration.
 * Nexus uses username/password authentication unlike token-based systems.
 */
export const EditMode: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(canvas.getByLabelText("URL", { selector: "input" })).toHaveValue("https://nexus-api.example.com");
    await expect(canvas.getByLabelText("User", { selector: "input" })).toHaveValue("nexus-admin");
    await expect(canvas.getByLabelText("Password", { selector: "input" })).toHaveValue("nexus-secret-password-123");
...
export const EditModeUndoChanges: Story = {
  ...
  play: async ({ canvas }) => {
    const urlInput = canvas.getByLabelText("URL", { selector: "input" });
    const originalValue = "https://nexus-api.example.com";
 ...
 * Verifies validation error messages on Quick Link URL and User fields.
 */
export const EditModeValidation: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await userEvent.tab();
    await expect(canvas.getByText("Enter the external Nexus URL.")).toBeInTheDocument();
    ...
    await userEvent.tab();
    await expect(canvas.getByText("Enter your Nexus username.")).toBeInTheDocument();
  },
};
