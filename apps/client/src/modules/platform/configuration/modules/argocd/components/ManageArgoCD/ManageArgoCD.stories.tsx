...
const manageArgoCDDecorator = withAppProviders({
...
const meta = {
  title: "Platform/Configuration/ArgoCD/ManageArgoCD",
  component: ManageArgoCD,
  parameters: {
    layout: "padded",
  },
  tags: ["autodocs"],
  decorators: [
    manageArgoCDDecorator,
    (Story) => (
...
} satisfies Meta<typeof ManageArgoCD>;
...
const mockSecret: Secret = {
  ...
  type: "Opaque",
  data: {
    token: btoa("argocd-secret-token-12345"),
    url: btoa("https://argocd-api.example.com"),
  },
};

/**
 * Create mode - New ArgoCD integration
 * Shows the form with no existing configuration, ready to create a new ArgoCD integration.
 */
 ...
 * Shows the form with an existing ArgoCD integration.
...
export const EditMode: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(canvas.getByLabelText("URL", { selector: "input" })).toHaveValue("https://argocd-api.example.com");
    await expect(canvas.getByLabelText("Token", { selector: "input" })).toHaveValue("argocd-secret-token-12345");
...
export const EditModeUndoChanges: Story = {
  ...
  play: async ({ canvas }) => {
    const urlInput = canvas.getByLabelText("URL", { selector: "input" });
    const originalValue = "https://argocd-api.example.com";
...
export const EditModeValidation: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await userEvent.tab();
    await expect(canvas.getByText("Enter the Argo CD URL.")).toBeInTheDocument();
