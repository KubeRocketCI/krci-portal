...
export const Actions = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result = await trpc.k8s.testIntegrationConnection.mutate({ serviceType: "argocd", url, token });
      if (cancelled) return;
      ...
      if (result.success) {
        ...
        switch (result.error) {
          ...
          case "NETWORK":
            ...
            break;
          case "HTTP_ERROR":
            setTestError(`Argo CD responded with status ${result.statusCode}.`);
            break;
        }
      }
    } catch (error) {
        ...
        <div className="my-2">
          {testStatus === "success" && (
            <AlertRoot className="border-green-500 bg-green-50 text-green-800 dark:bg-green-950 dark:text-green-200">
              <CheckCircle2 />
              <AlertDescription>Connection to Argo CD is working.</AlertDescription>
            </AlertRoot>
          )}
