...
export const CreateDefectDojoForm: React.FC<CreateDefectDojoFormProps> = ({ quickLink, onClose }) => {
  ...
  const handleSubmit = React.useCallback(
    async (values: CreateDefectDojoFormValues) => {
      const loadingToastId = showToast("Creating DefectDojo integration", "loading");
      try {
        ...
        showToast(result.data?.message || "DefectDojo integration created successfully", "success", {
          id: loadingToastId,
          duration: 5000,
        });
        onClose();
        ...
        showToast("Failed to create DefectDojo integration", "error", {
          id: loadingToastId,
          duration: 10000,
