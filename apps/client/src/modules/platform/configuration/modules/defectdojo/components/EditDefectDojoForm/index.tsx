...
import { EditDefectDojoFormValues } from "./types";
import { NAMES } from "./constants";
import { EditDefectDojoFormProvider } from "./providers/form/provider";
...
export type { EditDefectDojoFormProps } from "./types";

export const EditDefectDojoForm: React.FC<{
  secret: Secret;
  ...
  const defaultValues = React.useMemo<Partial<EditDefectDojoFormValues>>(
    () => ({
  ...
  return (
    <EditDefectDojoFormProvider defaultValues={defaultValues} onSubmit={handleSubmit} onSubmitError={handleSubmitError}>
      <DialogHeader>
        <div className="flex flex-row items-start justify-between gap-2">
          <div className="flex flex-col gap-4">
            <DialogTitle className="text-xl font-medium">Edit DefectDojo Integration</DialogTitle>
          </div>
      ...
      <DialogBody className="flex min-h-0">
        <div className="flex min-h-0 flex-1 gap-4">
          <div className="min-h-0 flex-1 overflow-y-auto p-0.5">
            <div className="flex flex-col gap-4">
              {requestError && (
                <Alert variant="destructive" title="Failed to update DefectDojo integration">
