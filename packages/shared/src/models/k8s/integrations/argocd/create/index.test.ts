...
import { createArgoCDIntegrationSecretDraft } from "./index.js";
...
describe("createArgoCDIntegrationSecretDraft", () => {
  it("should create a valid ArgoCD integration secret draft", () => {
    const input = {
      token: "test-token-123",
      url: "https://argocd.example.com",
    };

    const result = createArgoCDIntegrationSecretDraft(input);
  ...
  it("should throw ZodError when token is missing", () => {
    ...
    expect(() => createArgoCDIntegrationSecretDraft(input)).toThrow(ZodError);
  });
  ...
  it("should handle special characters in token and url", () => {
    const input = {
      ...
      url: "https://argocd.example.com:8080/path?query=value",
    };

    const result = createArgoCDIntegrationSecretDraft(input);

    expect(atob(result.data?.token as string)).toBe("token-with-special-chars!@#$%^&*()");
    expect(atob(result.data?.url as string)).toBe("https://argocd.example.com:8080/path?query=value");
  });
});
