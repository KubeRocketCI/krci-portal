...
export const editGitServer = (
  existingGitServer: GitServer,
  input: z.infer<typeof editGitServerSchema>
): GitServer => {
  return editResource(
    existingGitServer,
    input,
    editGitServerSchema,
    (draft: Draft<GitServer>, validatedInput) => {
      if (!draft.spec) {
        draft.spec = {} as any;
      }
      ...
      draft.spec.skipWebhookSSLVerification =
        validatedInput.skipWebhookSSLVerification;
