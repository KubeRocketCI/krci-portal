...
// Note: K8s API 401 stays UNAUTHORIZED so the client can redirect to login
// when K8s rejects the token (expired, revoked, clock skew, etc.)
// K8s API 403 is mapped to FORBIDDEN to distinguish RBAC denials from session expiration
...
export function handleK8sError(error: unknown): TRPCError {
  ...
  if (error instanceof K8sApiError) {
    ...
    return new TRPCError({
      ...
      code: trpcCode,
      cause: {
        source: "k8s", // Identifies error origin for server-side debugging
