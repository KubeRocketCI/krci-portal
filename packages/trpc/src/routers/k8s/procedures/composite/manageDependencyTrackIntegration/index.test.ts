...
vi.mock("@my-project/shared", async (importOriginal) => {
  ...
  return {
    ...
    editDependencyTrackIntegrationSecret: vi.fn((secret: Secret, input: { token: string; url: string }) => ({
      ...secret,
      data: { token: btoa(input.token), url: btoa(input.url) },
    })),
  };
});

describe("k8sManageDependencyTrackIntegrationProcedure", () => {
  ...
  const mockQuickLink: QuickLink = {
    ...
    kind: "QuickLink",
    metadata: { name: "dependency-track", namespace: "test-namespace" },
    spec: { type: "dependency-track", url: "https://dependency-track.example.com", icon: "dependency-track", visible: true },
  };

  const mockSecret: Secret = {
    apiVersion: "v1",
    kind: "Secret",
    metadata: { name: "ci-dependency-track", namespace: "test-namespace" },
    type: "Opaque",
    data: { token: btoa("test-token"), url: btoa("https://dependency-track.example.com") },
  };
  ...
  describe("create mode", () => {
    it("should create secret in create mode", async () => {
      const input = {
        ...
        secret: { token: "test-token", url: "https://dependency-track.example.com" },
      };
      mockK8sClientInstance.createResource.mockResolvedValueOnce(mockSecret);
      const caller = createCaller(mockContext);
      const result = await caller.k8s.manageDependencyTrackIntegration(input);
  ...
  describe("edit mode", () => {
    it("should only update secret when only secret is dirty", async () => {
      const input = {
        ...
        mode: "edit" as const,
        dirtyFields: { quickLink: false, secret: true },
        secret: { token: "updated-token", url: "https://dt-new.example.com", currentResource: mockSecret },
      };
      const updatedSecret = { ...mockSecret, data: { token: btoa("updated-token"), url: btoa("https://dt-new.example.com") } };
    ...
    it("should only update quickLink when only quickLink is dirty", async () => {
      const input = {
        ...
        quickLink: { name: "dependency-track", externalUrl: "https://dt-updated.example.com", currentResource: mockQuickLink },
        secret: { token: "test-token", url: "https://dependency-track.example.com", currentResource: mockSecret },
      };
      const updatedQuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://dt-updated.example.com" } };
    ...
    it("should update both resources when both are dirty", async () => {
      const input = {
        ...
        quickLink: { name: "dependency-track", externalUrl: "https://dt-updated.example.com", currentResource: mockQuickLink },
        secret: { token: "updated-token", url: "https://dt-updated.example.com", currentResource: mockSecret },
      };
      const updatedSecret: Secret = { ...mockSecret, data: { token: btoa("updated-token"), url: btoa("https://dt-updated.example.com") } };
      const updatedQuickLink: QuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://dt-updated.example.com" } };
      mockK8sClientInstance.replaceResource.mockResolvedValueOnce(updatedSecret).mockResolvedValueOnce(updatedQuickLink);
      const caller = createCaller(mockContext);
      const result = await caller.k8s.manageDependencyTrackIntegration(input);
      expect(result.success).toBe(true);
      expect(result.data.secret).toEqual(updatedSecret);
  ...
  describe("validation", () => {
    it("should require currentResource for secret in edit mode", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDependencyTrackIntegration(input as never)).rejects.toThrow(
    ...
    it("should require currentResource for quickLink in edit mode when dirty", async () => {
      const input = {
        ...
        mode: "edit" as const,
        dirtyFields: { quickLink: true, secret: false },
        quickLink: { name: "dependency-track", externalUrl: "https://dt.example.com" },
        secret: { token: "test-token", url: "https://dependency-track.example.com", currentResource: mockSecret },
      };
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDependencyTrackIntegration(input as never)).rejects.toThrow(
  ...
  describe("fail-fast behavior", () => {
    it("should stop execution if first operation fails", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDependencyTrackIntegration(input)).rejects.toThrow("Failed to create secret");
    });
    ...
    it("should leave first operation completed if second operation fails", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDependencyTrackIntegration(input)).rejects.toThrow("Failed to update QuickLink");
      expect(mockK8sClientInstance.replaceResource).toHaveBeenCalledTimes(2);
    });
  });
