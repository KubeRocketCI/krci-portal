...
const baseInput = {
  ...
  secret: { token: "test-token", url: "https://deptrack.example.com" },
};
...
describe("k8sManageDependencyTrackIntegrationProcedure", () => {
  ...
  it("creates the DependencyTrack secret from token and url", async () => {
    k8s.createResource.mockResolvedValueOnce(written);

    const result = await createCaller(mockContext).k8s.manageDependencyTrackIntegration({
      ...baseInput,
      mode: "create",
      dirtyFields: { quickLink: false, secret: true },
    });

    expect(createDependencyTrackIntegrationSecretDraft).toHaveBeenCalledWith({
      token: "test-token",
  ...
  it("patches the live DependencyTrack secret with token and url", async () => {
    k8s.replaceResource.mockResolvedValueOnce(written);

    await createCaller(mockContext).k8s.manageDependencyTrackIntegration({
      ...baseInput,
      dirtyFields: { quickLink: false, secret: true },
      secret: { ...baseInput.secret, currentResource: secret },
    });

    expect(editDependencyTrackIntegrationSecret).toHaveBeenCalledWith(secret, {
      token: "test-token",
      url: "https://deptrack.example.com",
    });
  ...
  it("rewrites the QuickLink url", async () => {
    ...
    await createCaller(mockContext).k8s.manageDependencyTrackIntegration({
      ...
      quickLink: { name: "deptrack", externalUrl: "https://deptrack-updated.example.com", currentResource: quickLink },
    });

    expect(editQuickLinkURL).toHaveBeenCalledWith(quickLink, { url: "https://deptrack-updated.example.com" });
