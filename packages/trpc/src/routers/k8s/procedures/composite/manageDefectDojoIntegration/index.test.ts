...
vi.mock("@my-project/shared", async (importOriginal) => {
  ...
  return {
    ...
    editDefectDojoIntegrationSecret: vi.fn((secret: Secret, input: { token: string; url: string }) => ({
      ...secret,
...
describe("k8sManageDefectDojoIntegrationProcedure", () => {
  ...
  const mockQuickLink: QuickLink = {
    ...
    kind: "QuickLink",
    metadata: { name: "defect-dojo", namespace: "test-namespace" },
    spec: { type: "defect-dojo", url: "https://defectdojo.example.com", icon: "defect-dojo", visible: true },
  };

  const mockSecret: Secret = {
    apiVersion: "v1",
    kind: "Secret",
    metadata: { name: "ci-defect-dojo", namespace: "test-namespace" },
    type: "Opaque",
    data: { token: btoa("test-token"), url: btoa("https://defectdojo.example.com") },
  };
  ...
  describe("create mode", () => {
    it("should create secret in create mode", async () => {
      ...
      const result = await caller.k8s.manageDefectDojoIntegration(input);
  ...
  describe("edit mode", () => {
    it("should only update secret when only secret is dirty", async () => {
      const input = {
        ...
        mode: "edit" as const,
        dirtyFields: { quickLink: false, secret: true },
        secret: { token: "updated-token", url: "https://dd-new.example.com", currentResource: mockSecret },
      };
      const updatedSecret = { ...mockSecret, data: { token: btoa("updated-token"), url: btoa("https://dd-new.example.com") } };
    ...
    it("should only update quickLink when only quickLink is dirty", async () => {
      const input = {
        ...
        quickLink: { name: "defect-dojo", externalUrl: "https://dd-updated.example.com", currentResource: mockQuickLink },
        secret: { token: "test-token", url: "https://defectdojo.example.com", currentResource: mockSecret },
      };
      const updatedQuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://dd-updated.example.com" } };
    ...
    it("should update both resources when both are dirty", async () => {
      const input = {
        ...
        quickLink: { name: "defect-dojo", externalUrl: "https://dd-updated.example.com", currentResource: mockQuickLink },
        secret: { token: "updated-token", url: "https://dd-updated.example.com", currentResource: mockSecret },
      };
      const updatedSecret: Secret = { ...mockSecret, data: { token: btoa("updated-token"), url: btoa("https://dd-updated.example.com") } };
      const updatedQuickLink: QuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://dd-updated.example.com" } };
  ...
  describe("validation", () => {
    it("should require currentResource for secret in edit mode", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDefectDojoIntegration(input as never)).rejects.toThrow(
    ...
    it("should require currentResource for quickLink in edit mode when dirty", async () => {
      const input = {
        ...
        mode: "edit" as const,
        dirtyFields: { quickLink: true, secret: false },
        quickLink: { name: "defect-dojo", externalUrl: "https://defectdojo.example.com" },
        secret: { token: "test-token", url: "https://defectdojo.example.com", currentResource: mockSecret },
      };
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDefectDojoIntegration(input as never)).rejects.toThrow(
  ...
  describe("fail-fast behavior", () => {
    it("should stop execution if first operation fails", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDefectDojoIntegration(input)).rejects.toThrow("Failed to create secret");
    });
    ...
    it("should leave first operation completed if second operation fails", async () => {
      ...
      const caller = createCaller(mockContext);
      await expect(caller.k8s.manageDefectDojoIntegration(input)).rejects.toThrow("Failed to update QuickLink");
      expect(mockK8sClientInstance.replaceResource).toHaveBeenCalledTimes(2);
    });
  });
