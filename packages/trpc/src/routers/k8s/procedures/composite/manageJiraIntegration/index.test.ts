...
describe("k8sManageJiraIntegrationProcedure", () => {
  ...
  const mockQuickLink: QuickLink = {
    ...
    kind: "QuickLink",
    metadata: { name: "jira", namespace: "test-namespace" },
    spec: { type: "jira", url: "https://jira.example.com", icon: "jira", visible: true },
  };

  const mockSecret: Secret = {
    apiVersion: "v1",
    kind: "Secret",
    metadata: { name: "ci-jira", namespace: "test-namespace" },
    type: "Opaque",
  ...
  describe("edit mode", () => {
    it("should only update secret when only secret is dirty", async () => {
      ...
      const updatedSecret = { ...mockSecret, data: { username: btoa("updated-user"), password: btoa("updated-password") } };
    ...
    it("should only update quickLink when only quickLink is dirty", async () => {
      ...
      const updatedQuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://jira-updated.example.com" } };
    ...
    it("should update both resources when both are dirty", async () => {
      ...
      const updatedSecret: Secret = { ...mockSecret, data: { username: btoa("updated-user"), password: btoa("updated-password") } };
      const updatedQuickLink: QuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://jira-updated.example.com" } };
