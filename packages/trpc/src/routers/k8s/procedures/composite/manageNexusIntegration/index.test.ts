...
vi.mock("@my-project/shared", async (importOriginal) => {
  ...
  return {
    ...
    editNexusIntegrationSecret: vi.fn(
      (secret: Secret, input: { username: string; password: string; url: string }) => ({
        ...secret,
        data: {
...
describe("k8sManageNexusIntegrationProcedure", () => {
  ...
  const mockQuickLink: QuickLink = {
    ...
    kind: "QuickLink",
    metadata: { name: "nexus", namespace: "test-namespace" },
    spec: { type: "nexus", url: "https://nexus.example.com", icon: "nexus", visible: true },
  };

  const mockSecret: Secret = {
    apiVersion: "v1",
    kind: "Secret",
    metadata: { name: "ci-nexus", namespace: "test-namespace" },
    type: "Opaque",
  ...
  describe("edit mode", () => {
    ...
    it("should only update quickLink when only quickLink is dirty", async () => {
      const input = {
        ...
        secret: { username: "test-user", password: "test-password", url: "https://nexus.example.com", currentResource: mockSecret },
      };
      const updatedQuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://nexus-updated.example.com" } };
    ...
    it("should update both resources when both are dirty", async () => {
      ...
      const updatedQuickLink: QuickLink = { ...mockQuickLink, spec: { ...mockQuickLink.spec, url: "https://nexus-updated.example.com" } };
